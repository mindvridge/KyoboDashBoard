...
- **디바이스 기반 자동 인증**: 디바이스 등록 시 JWT 토큰 발급
- 사용자 로그인 불필요 - 디바이스 ID와 MAC 주소로 자동 인증
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| device_id | string | O | 고유 디바이스 식별자 |
| mac_address | string | O | MAC 주소 |
...
|------|------|------|------|
| session_id | string | O | 세션 UUID |
| content_id | string | O | 콘텐츠 ID (max 255자) |
| content_name | string | O | 콘텐츠 이름 (max 500자) |
...
|------|------|------|------|
| session_id | string | O | 세션 UUID |
| content_id | string | O | 콘텐츠 ID |
| content_name | string | O | 콘텐츠 이름 |
| action_type | string | O | WATCH_START / WATCH_END / WATCH_PAUSE / WATCH_RESUME |
...
public class Device
{
    ...
    public string last_seen;    // 마지막 활동 시간
}
```
...
public class ContentLog
{
    ...
    public string action_type;  // 액션 타입
...
1. 디바이스 등록 (POST /api/devices/register)
   - device_id와 mac_address로 자동 등록
           ↓
2. JWT 토큰 수신 (response.data.token)
           ↓
...
| POST /api/devices/heartbeat | O |
| POST /api/sessions/start | O |
| POST /api/sessions/end | O |
| POST /api/logs/* | O |
| GET /api/* (조회) | - |
...
// 2. 토큰은 VRLogger가 메모리에서 자동 관리

// 3. 공간(Space) 할당 - 첫 등록 시 space_id 지정
```
...
public class VRAppManager : MonoBehaviour
{
    ...
    {
        if (isWatching) await StopPlaybackAsync();
    ...
    {
        Debug.Log($"디바이스 등록: {(success ? "성공" : "실패")}");
        if (success) _ = VRLogger.Instance.LogLobbyEnter();
    }
...
public class VideoManager : MonoBehaviour
{
    ...
    public Video[] GetAllVideos() => videos;
    public int GetVideoCount() => videos?.Length ?? 0;
}
```
...
| 문제 | 원인 | 해결 |
|------|------|------|
| 등록 실패 | 서버 연결 불가 | 서버 URL 확인 |
| 세션 시작 안됨 | 등록 안됨 | AutoLogin 확인 |
...
| 버전 | 날짜 | 변경 내용 |
|------|------|----------|
| 2.0.0 | 2025-12-01 | 문서 전면 개정 - 디바이스 자동 등록/로그 기능 중심으로 단순화 |
| 1.2.0 | 2025-12-01 | 비디오 is_preinstalled 필드 추가 |
